...
*/
function FeatherImportBundlerCSR(imports, seperator = "\n") {
...
* @returns {string | undefined}
*/
function FeatherImportBundlerSSR(imports, seperator = "\n") {
    const bundel = imports.map((path) => {
...
export class FeatherDocument {
        ...
        if (MatchedProps && properties) {
            for (var property of MatchedProps) {
                ...
                if (value.TrustKey === this.TrustKey) {
                    this.raw = this.raw.replace(new RegExp(property.text, "g"), value.raw);
                    console.log(this.raw);
                }
                else {
                    this.raw = this.raw.replace(new RegExp(property.text, "g"), EscapeHTML(value));
                }
            }
        }
    }
    ...
    _GetTag_(tag) {
        ...
        if (!tags || tags.length < 1)
            return;
        if (tags.length > 1)
            console.warn(config.messages.TagConfict, tag);
        return tags[0];
    }
    _GetTags_(tag) {
        const tags = [
            ...this.raw.matchAll(new RegExp(config.regex.TagMatch.replace(new RegExp("{TAG}", "g"), tag), config.regex.TagMatchFlags))
        ];
        if (!tags)
            return;
    ...
    _EvaluateJavascript_(props = {}, ContainProps = true) {
        if (ContainProps) {
...
export default class FeatherJS {
    BakeElementFromString;
    ModuleBundel;
    CssBundel;
    options;
    constructor(ModuleImportPaths = [], CssImportPaths = [], options = {
        TrustKey: "UUID",
        ssr: false
    }) {
        this.options = options;
        if (this.options.TrustKey === "UUID")
            this.options.TrustKey = UUID();
        ...
        this.ModuleBundel = this.BakeElementFromString(LocalImportBundler(ModuleImportPaths));
        this.CssBundel = LocalImportBundler(CssImportPaths);
    }
    /**
    * @returns {FeatherDocument | undefined}
    */
    BakeElement(tag, InitialProps) {
        ...
        if (ScriptTag) {
            AdaptedProps = Object.assign(AdaptedProps, ScriptTag.document._EvaluateJavascript_(InitialProps));
